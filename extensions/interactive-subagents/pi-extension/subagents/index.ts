...
interface RunningSubagent {
  ...
  agent?: string;
  surface: string;
  /** Owned top-level branch tab; nested surfaces share their parent's tab. */
  herdrTabId?: string;
...
function applySandboxToParts(
  ...
  if (loadout.toolAllowlist) {
    ...
    const childAgentDir = loadout.agentDir ?? process.env.PI_CODING_AGENT_DIR ?? join(homedir(), ".pi", "agent");
    const herdrStatePath = getHerdrIntegrationPath(getAgentConfigDir());
    if (
      process.env.HERDR_ENV === "1" &&
      existsSync(herdrStatePath) &&
      (loadout.toolAllowlist || resolve(childAgentDir) !== resolve(getAgentConfigDir()))
    ) {
...
async function launchSubagent(
  ...
  if (agentDefs?.cli === "claude") {
    ...
    const allocatedSurface = createSubagentSurface(params.name, params.task, {
...
export default function subagentsExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
      ...
      async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
        ...
        const allocatedSurface = createSubagentSurface(name, message || "Resume subagent", {
  ...
  pi.registerMessageRenderer("subagent_result", (message, options, theme) => {
    ...
    return {
      render(width: number): string[] {
        ...
        // Remove the leading result label for display.
