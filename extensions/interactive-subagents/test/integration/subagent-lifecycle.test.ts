 ...
 * Each test creates a dedicated Herdr tab, runs Pi with a task that uses the subagent
 ...
 *
 * Run inside Herdr from a Pi pane:
...
for (const backend of backends) {
  describe(`subagent-lifecycle [${backend}]`, { timeout: PI_TIMEOUT * 3 }, () => {
    ...
    it("spawns a subagent that writes a file and verifies the session", async () => {
      const id = uniqueId();
      const markerFile = join(env.dir, `pi-integ-echo-${id}.txt`);
    ...
    it("keeps a long active tool call from surfacing false stalled status", async () => {
      const id = uniqueId();
      const startFile = join(env.dir, `pi-integ-status-start-${id}.txt`);
      const markerFile = join(env.dir, `pi-integ-status-${id}.txt`);
    ...
    it("spawns two subagents in parallel and both complete", async () => {
      const id = uniqueId();
      const fileA = join(env.dir, `pi-integ-para-${id}-a.txt`);
      const fileB = join(env.dir, `pi-integ-para-${id}-b.txt`);
    ...
    it("fork mode creates a child session linked to the parent", async () => {
      const id = uniqueId();
      const markerFile = join(env.dir, `pi-integ-fork-${id}.txt`);
      trackTempFile(env, markerFile);
    ...
    it("subagent discovers project-local test agents", async () => {
      const id = uniqueId();
      const markerFile = join(env.dir, `pi-integ-discovery-${id}.txt`);
      trackTempFile(env, markerFile);
    ...
    it("passes systemPrompt to subagent", async () => {
      const id = uniqueId();
      const markerFile = join(env.dir, `pi-integ-sysprompt-${id}.txt`);
      trackTempFile(env, markerFile);
