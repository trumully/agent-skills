...
describe("subagent discovery", () => {
  ...
  it("worker is granted the spawning toolset restricted to scout", () => {
    const worker = testApi.loadAgentDefaults("worker");
    assert.ok(worker, "expected bundled worker to be discoverable");
    assert.deepEqual(worker.subagentAgents, ["scout"]);
  ...
  it("resolves web research tools to the installed pi-web-access extension", () => {
    withTempDir((dir) => {
      const originalAgentDir = process.env.PI_CODING_AGENT_DIR;
      process.env.PI_CODING_AGENT_DIR = dir;
      try {
        const extensionDirectory = join(dir, "npm", "node_modules", "pi-web-access", "dist");
        mkdirSync(extensionDirectory, { recursive: true });
        const extensionPath = join(extensionDirectory, "index.js");
        writeFileSync(extensionPath, "");

        for (const tool of ["web_search", "source_check", "fetch_content", "get_search_content"]) {
          assert.equal(testApi.getToolExtensionPath(tool), extensionPath);
        }
      } finally {
  ...
  it("applySandboxToParts replays model, identity, and default-deny tool restriction", () => {
    withTempDir((d) => {
      ...
      // Herdr passes argv directly, so the value remains unquoted.
      assert.ok(
  ...
  it("buildPiPromptArgs submits skill commands before the direct task text", () => {
    const task = "Do the task.\nKeep this line intact.";
    assert.deepEqual(
      testApi.buildPiPromptArgs({ effectiveSkills: "review,lint", taskArg: task }),
      ["/skill:review", "/skill:lint", task],
    );
  });

  it("buildPiPromptArgs submits the task directly when there are no skills", () => {
    assert.deepEqual(
      testApi.buildPiPromptArgs({ effectiveSkills: undefined, taskArg: "do the task" }),
      ["do the task"],
    );
  });
...
describe("subagent interruption", () => {
  ...
  it("steers a running subagent with a Herdr prompt (newlines flattened)", () => {
    const testApi = (subagentsModule as any).__test__;
    let sentSurface = "";
    let sentText = "";
  ...
  it("returns an explicit error when steering delivery fails", () => {
    ...
    const result = testApi.steerSubagent(running, "hi", () => {
      throw new Error("Herdr prompt failed");
    });
...
describe("Herdr agent names", () => {
  const testApi = (subagentsModule as any).__test__;

  it("creates a valid, bounded, unique Herdr alias from the display name and ID", () => {
    const alias = testApi.herdrAgentNameFor("Worker / Review", "a1b2c3d4");
    assert.equal(alias, "sub-worker-review-a1b2c3d4");
    assert.match(alias, /^[a-z][a-z0-9_-]{0,31}$/);
    assert.ok(testApi.herdrAgentNameFor("😀", "12345678").length <= 32);
  });
});
