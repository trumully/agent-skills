...
Mix-and-match single-purpose prompt snippets that are prepended or appended to your message when you send it.
...
- Press `Alt+S` or run `/snippets` to open the toggle menu.
  ...
  - The menu is framed with top/bottom border lines and scrolls when the list exceeds the viewport (max height adapts to your terminal), with `↑ n more` / `↓ n more` indicators when clipped.
...
- When you send a message, active snippet bodies are merged into the message text: prepend group (sorted by `order`) → your text → append group (sorted by `order`), separated by blank lines. 
- Toggles reset to **all off** after each send and at session start.

## Snippet files

Snippets live in `snippets/` next to `index.ts` one markdown file each, with frontmatter:
...
Files are re-scanned every time the menu opens and every time a message is sent, so edits take effect immediately no `/reload` needed.