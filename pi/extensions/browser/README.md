...
After copying the extension, install its locked dependencies. These commands work in Linux Bash and Windows PowerShell:

```sh
cd "$HOME/.pi/agent/extensions/browser"
npm ci
npx --no-install playwright-core install chromium
```

Use the local `playwright-core` CLI so the downloaded Chromium matches the version in `package-lock.json`. On Linux, install Chromium's system libraries too if they are missing:

```bash
npx --no-install playwright-core install --with-deps chromium
```

The Linux system-library step can require administrator privileges and a supported distribution. Windows uses native Chromium; WSL is not required. Install dependencies and browser binaries separately on each OS rather than copying `node_modules` or browser profiles between them.
...
- `playwright-core` ships without browser binaries; run its local install command
  above on each machine and again after updating the locked Playwright version.
