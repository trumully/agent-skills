...
All three use `thinking: max`, are autonomous (`auto-exit: true`), and carry their identity in the system prompt (`system-prompt: append`).
...
Use [psmux](https://github.com/psmux/psmux) and Git for Windows. Put these settings in `$HOME/.psmux.conf`, not `.psmux/.tmux.conf`:

```tmux
set -g default-shell "C:/Program Files/Git/bin/bash.exe" --login -i
set -g warm off
```

The login shell supplies Git's Bash tools on PATH. Disabling prewarming avoids stale shell environments and `__warm__` session identities. Keep Node, Pi, and psmux on PATH.

From a new terminal outside psmux, run `tmux new-session -s pi-native`, then start `pi` at its Git Bash prompt. An existing pane retains its old environment; `/reload` alone cannot correct a missing `sh.exe` on PATH.
