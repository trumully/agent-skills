...
// Invoke Mermaid CLI's JavaScript entry through the current Node executable.
// Calling node_modules/.bin/mmdc directly is not portable on Windows because
// that file is a shell wrapper (mmdc.cmd is required there).
    ...
    async execute(_id, params) {
      ...
      if (res.code !== 0 || !existsSync(outPath)) {
        ...
        const note = res.timedOut
          ? "mmdc timed out.\n\n"
          : !existsSync(MMDC_CLI)
            ? `Mermaid CLI is not installed at ${MMDC_CLI}. Run npm install in the visual-tools directory.\n\n`
            : !chrome
              ? "No Chrome/Chromium executable was detected. Install one or set PI_VISUAL_CHROME_PATH.\n\n"
              : ""
        return {
          content: [
            {
