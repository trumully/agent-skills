 ...
 *
 * Prefer the system renderers because they generally have better access to
 * installed fonts. The bundled resvg fallback keeps the maker usable on a
 * clean Windows machine, where rsvg-convert is uncommon and ImageMagick may
 * not be installed.
 */
async function renderSvg(svgPath: string, outPath: string, workDir: string) {
  ...
  // ImageMagick. -density 192 (~2x of 96dpi) for a crisp raster.
  ...
  // Portable final fallback: @resvg/resvg-js ships platform binaries and does
  // not require a system executable. It is intentionally loaded lazily so a
  // missing optional native package cannot prevent the extension from loading.
  try {
    const { Resvg } = await import("@resvg/resvg-js")
    const svg = readFileSync(svgPath, "utf8")
    const png = new Resvg(svg).render().asPng()
    writeFileSync(outPath, png)
    return { ok: true as const, res: { code: 0, stdout: "", stderr: "", timedOut: false } }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    return {
      ok: false as const,
      res: {
        code: rsvg.code !== null ? rsvg.code : magick.code,
        stdout: [rsvg.stdout, magick.stdout].filter(Boolean).join("\n"),
        stderr: [rsvg.stderr, magick.stderr, `resvg: ${message}`].filter(Boolean).join("\n"),
        timedOut: rsvg.timedOut || magick.timedOut,
      },
    }
  }
}
      ...
      if (!ok) {
        ...
        return {
          content: [
            {
              type: "text",
              text: `${note}SVG render FAILED — no image produced (tried rsvg-convert, magick, and bundled resvg). Fix the source with edit_svg and call render_svg again.\n\nError:\n${detail}`,
            },
          ],
